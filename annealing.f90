PROGRAM lia
  ...
  !!
  integer :: i, j, c_num(288), n1, n2, flag, index_switch
  real(kind=8) :: crd_Mo(576,3), crd_C(576,3), x, a(3,3)
  real(kind=8) , dimension(:,:) :: coord_Mo(576,3),coord_C(576,4)
  !integer :: fid
  !character (len=5) :: fileno, charI
  !character (len=20):: foutname
  !!
  open(unit=86,file="moc-466.vasp")    !archivo que contiene las coordenadas de Mo
  ...
  !coment. importante      segun el diagrama de la oficina la temp. maxima que alcanzan el alfa-mo2c es 1430°C, desp de eso sigu la fase beta-mo2c
  !coment. importante      por lo que en teoria la temp. maxima deberia ser 1703.15 K  |  K = °C + 273.15


  imax=120000           !ciclos, cycles (iterations)
  temp1=300
  temp2=2000
  ...
  do i=1,576                         ! recordar la forma extrana de fortran de leer los files, ya que se pueden usar dos veces el mismo
...
!!!!!!!!!!!!!!!!!!! generating first random structure 16 C atoms vacancy and 16 occupency !!!!!!!!!!!!!!!!!!!
  ...
  do i=1,3000
     ...
     n1=int((x*576.0)) + 1           ! que se asigna a la variable n1 | IB
     !write(*,*) n1
     ...
     if (flag.eq.2 .and. n2.eq.288) then
        exit
     endif
  enddo
  ...
  do i=1,288                     ! aca las coordenadas que esten en la posiciones de c_num(i) van a ser cero
...
!!!!!!!!!! END !!!!!!!!!!
  ...
  energy2=0.0
  op=0.0
  do temp=temp1,temp2,100
     do iterations=1,imax
        energy2=0.0

        call random_number (x) !            ! 100
        n1=int(x*576.0) + 1  ! 1st index
        call random_number (x)
        n2=int(x*576.0) + 1  ! 2nd index
